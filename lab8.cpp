...
int main(int argc, char *argv[])
{
	SBSI sbsi;
	//
    //if command line argument present, run unit-test
    //use a tolerance value of 0.005
    if (argc > 1) {//TWO ARGUMENTS, 1 to start, next is tolerance val
       cout << "Unit test for SBSI function...\n";
       cout << "tolerance: " << argv[2] << "\n";
       inputData();
       cout << "H" << setw(5) << " WC" << setw(4) << "BSA" << setw(4)
            << "VTC" << setw(8) << "SBSI" << setw(16) << "Function\n";
       cout << "--- "  << "-- " <<  "---- " << "--- " << "  ------"
            << "    -------\n";
       showData(argv[2]);
      cout << endl;
     cout << "bad element count:" << badElementCount << endl;
...
{
    for (int i = 0; i < 81; i++) {
       sbsiArray[i].h = data[i*5+0];
       sbsiArray[i].wc = data[i*5+1];
       sbsiArray[i].bsa = data[i*5+2];
       sbsiArray[i].vtc = data[i*5+3];
    }
}
